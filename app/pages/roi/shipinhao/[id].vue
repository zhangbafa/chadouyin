...
const handleSubmit = async (event: FormSubmitEvent<Schema>) => {
    ...
    try {
        ...
        // 2. 调用服务端 API 进行计算
        const data = await $fetch('/api/roi/shipinhao', {
            method: 'POST',
        ...
        results.value = {
            ...
            estimatedBreakEvenROI: data.estimatedBreakEvenROI,
            estimatedSettlementBreakEvenROI: data.estimatedSettlementBreakEvenROI,
            chujiaArr: data.chujiaArr,
            roiArr: data.roiArr,
            roijia: data.roijia,
            jianyijia: data.jianyijia
        }
        calculateTime.value = data.calculateTime
